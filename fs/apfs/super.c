...
#define APFS_MODULE_ID_STRING	"linux-apfs"
...
struct apfs_fs_context {
	struct apfs_sb_info *sbi;	/* Allocated early, transferred to sb on success */
	bool readwrite;
	bool cknodes;
};
...
static int apfs_parse_param(struct fs_context *fc, struct fs_parameter *param)
{
	...
	struct apfs_sb_info *sbi = ctx->sbi;
	...
	case Opt_cknodes:
		...
		break;
	case Opt_uid:
		sbi->s_uid = make_kuid(current_user_ns(), result.uint_32);
		if (!uid_valid(sbi->s_uid))
			return invalfc(fc, "invalid uid");
		break;
	case Opt_gid:
		sbi->s_gid = make_kgid(current_user_ns(), result.uint_32);
		if (!gid_valid(sbi->s_gid))
...
static int apfs_fill_super(struct super_block *sb, struct fs_context *fc)
{
	...
	/* Apply parsed mount options to container flags */
	if (ctx->readwrite)
...
/**
 * apfs_test_super_fc - Check if two volume superblocks are for the same volume
 * @sb:		superblock structure for a currently mounted volume
 * @fc:		filesystem context for the volume being mounted
 */
static int apfs_test_super_fc(struct super_block *sb, struct fs_context *fc)
{
	struct apfs_fs_context *ctx = fc->fs_private;
	struct apfs_sb_info *sbi_new = ctx->sbi;
	struct apfs_sb_info *sbi_old = APFS_SB(sb);

	if (sbi_new->s_nxi != sbi_old->s_nxi)
		return false;
	if (sbi_new->s_vol_nr != sbi_old->s_vol_nr)
		return false;
	return apfs_strings_are_equal(sbi_new->s_snap_name, sbi_old->s_snap_name);
}

/**
 * apfs_set_super_fc - Assign the device and an info struct to a superblock
...
static int apfs_get_tree(struct fs_context *fc)
{
	...
	error = apfs_attach_nxi(sbi, fc->source, mode);
	if (error)
		return error;
...
static void apfs_free_fc(struct fs_context *fc)
{
	...
	if (ctx->sbi) {
		kfree(ctx->sbi->s_snap_name);
		kfree(ctx->sbi->s_tier2_path);
		kfree(ctx->sbi);
	}
	kfree(ctx);
}
...
static void apfs_kill_sb(struct super_block *sb)
{
	 ...
	 * race with apfs_test_super_fc(), but we can't call
	 * kill_super_notify() from the driver. The available wrapper is
	 * kill_anon_super(), but our s_dev is set to the actual device (that
	 * gets freed later along with the container), not to the anon device
	 * that we keep on the sbi. So, we change that before the call; this
	 * is safe because other mounters won't revive this super, even if
	 * apfs_test_super_fc() succeeds.
	 */
